...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// Since the external is located in the global scope, it can be accessed. The internal variable is allowed to be accessed due to it's lexical scope
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(zooAnimal){
    const mappedArr = zooAnimal.map(function(zooAnimal){
...
class CuboidMakerTwo{
  ...
  surfaceArea(length, width, height, surfaceArea){
    return this.surfaceArea = 2 * 
    (this.length * this.width +
       this.length * this.height +
        this.width * this.height);
  };
